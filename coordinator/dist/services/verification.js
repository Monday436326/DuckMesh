...
class VerificationService {
    constructor() {
        ...
        this.bedrockRuntime = new aws_sdk_1.default.BedrockRuntime({ region });
        this.s3 = new aws_sdk_1.default.S3({ region });
    }
    async verifyResult(job, results, verificationMode) {
        switch (verificationMode) {
            case types_1.VerificationMode.Redundant:
                return this.verifyRedundant(results);
            case types_1.VerificationMode.ReferenceCheck:
                return this.verifyReference(job, results[0]);
            case types_1.VerificationMode.Attestation:
                return this.verifyAttestation(results[0]);
            default:
                return { isValid: true };
        }
    }
    async verifyRedundant(results) {
        if (results.length < 2) {
    ...
    async verifyReference(job, result) {
        try {
    ...
    async verifyAttestation(result) {
        try {
    ...
    async getBedrockInference(modelId, jobSpec) {
        try {
    ...
    async validateTeeAttestation(signature) {
